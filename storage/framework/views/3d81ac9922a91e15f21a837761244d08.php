            ...
            <div class="bg-white rounded-lg shadow p-4 mb-6">
                ...
                <?php if(!empty($ujianTA)): ?>
                    ...
                    <div class="bg-white rounded-lg shadow p-4">
                        ...
                        <div id="statusPendaftaranCard" class="px-3 py-4 rounded <?php echo e($currentPendaftaran['bg']); ?>">
                            <div id="statusPendaftaranLabel" class="font-semibold <?php echo e($currentPendaftaran['text']); ?>"><?php echo e($currentPendaftaran['label']); ?></div>
                        ...
                        <div class="mt-4 text-center">
                            <?php if(!empty($ujianTA) && (strpos(strtolower($ujianTA->status_ujian ?? ''), 'selesai') !== false)): ?>
                                <a id="hasilLink" href="<?php echo e(route('mahasiswa.ujian-ta.hasil')); ?>" class="inline-block px-4 py-2 bg-indigo-600 text-white rounded shadow text-sm hover:bg-indigo-700">Lihat Hasil</a>
                            <?php else: ?>
                                <a id="hasilLink" href="<?php echo e(route('mahasiswa.ujian-ta.hasil')); ?>" class="inline-block px-4 py-2 bg-indigo-600 text-white rounded shadow text-sm hover:bg-indigo-700 hidden">Lihat Hasil</a>
                            <?php endif; ?>
                            <span id="hasilHint" class="inline-block px-4 py-2 bg-gray-200 text-gray-700 rounded text-sm <?php if(!empty($ujianTA) && (strpos(strtolower($ujianTA->status_ujian ?? ''), 'selesai') !== false)): ?> hidden <?php endif; ?>">Hasil akan tersedia setelah ujian selesai</span>
                        </div>
                    </div>
                <?php else: ?>
                    <?php
...
?>
                ...
                <script>
                    ...
                    (function () {
                        const ujianId = <?php echo e(json_encode($ujianTA->id_ujian ?? null)); ?>;
                        if (!ujianId) return;

                        async function fetchStatus() {
                            try {
                                const res = await fetch(`<?php echo e(url('/mahasiswa/ujian-ta')); ?>/${ujianId}/status`, { headers: { 'Accept': 'application/json' } });
                                if (!res.ok) return;
                                ...
                                if (hasilHint && hasilLink) {
                                    if (isFinished) {
                                        ...
                                        hasilLink.setAttribute('href', '<?php echo e(route('mahasiswa.ujian-ta.hasil')); ?>');
                                    } else {
