...
<body class="bg-blue-50 text-gray-800">
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto p-6" x-data="reviewModal()">
                    ...
                    <div class="mb-6 bg-white rounded-xl shadow-sm p-4">
                        <div class="flex justify-between items-center">
                            <div>
                                ...
                                <p class="text-sm text-gray-600 mt-1">Mahasiswa Aktif: <span id="dospemMahasiswaAktif" class="font-semibold"><?php echo e($mahasiswaAktifCount ?? 0); ?></span> | Tugas Review: <span id="dospemTugasReview" class="font-semibold"><?php echo e($tugasReview ?? 0); ?></span></p>
                            </div>
                        </div>
                    </div>
                ...
                <!-- Modal -->
                <div x-show="showModal" x-cloak :hidden="!showModal" @keydown.escape.window="showModal = false" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                    <div class="flex items-end justify-center min-h-screen px-4 text-center md:items-center sm:block sm:p-0">
                        <div @click="showModal = false" x-show="showModal" x-cloak :hidden="!showModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true"></div>

                        <div x-show="showModal" x-cloak :hidden="!showModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block w-full max-w-lg p-8 my-20 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-lg">
    ...
    <script>
        ...
        function refreshReviewData() {
            fetch('<?php echo e(route("dospem.review-tugas.data")); ?>', {
                method: 'GET',
                headers: {
                    'Accept': 'application/json',
