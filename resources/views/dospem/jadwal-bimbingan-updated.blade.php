            ...
            <main class="flex-1 overflow-y-auto p-6">
                <div class="max-w-7xl mx-auto">
                    ...
                    <div class="mb-6 flex space-x-2 border-b border-gray-200">
                        ...
                        <button onclick="switchJadwalTab('calendar')" class="jadwal-tab-btn px-4 py-3 border-b-2 border-transparent text-gray-600 hover:text-gray-800 font-medium" data-tab="calendar">
                    ...
                    <div id="list-tab" class="jadwal-tab-content bg-white rounded-xl shadow-sm p-6">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <div class="flex items-center space-x-2">
                                <input type="text" id="searchInput" placeholder="Cari jadwal..." class="px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                        </div>
                        ...
                        <div class="mb-4 flex flex-wrap gap-2">
                            ...
                            <button onclick="filterJadwalStatus('menunggu')" class="filter-btn px-4 py-2 rounded-md text-sm bg-white text-gray-700 border border-gray-300 hover:border-yellow-300 hover:bg-yellow-50" data-status="menunggu">
                                <i class="fas fa-clock mr-1"></i>Menunggu
                            </button>
                            <button onclick="filterJadwalStatus('disetujui')" class="filter-btn px-4 py-2 rounded-md text-sm bg-white text-gray-700 border border-gray-300 hover:border-green-300 hover:bg-green-50" data-status="disetujui">
                                <i class="fas fa-check mr-1"></i>Disetujui
                            </button>
                            <button onclick="filterJadwalStatus('ditolak')" class="filter-btn px-4 py-2 rounded-md text-sm bg-white text-gray-700 border border-gray-300 hover:border-red-300 hover:bg-red-50" data-status="ditolak">
                                ...
                                <div class="border rounded-lg p-4 bg-white hover:bg-gray-50 transition flex justify-between items-start jadwal-item" data-status="{{ $jadwal->status ?? 'menunggu' }}">
                                    ...
                                    <div class="ml-4 flex gap-2">
                                        @if(($jadwal->status ?? 'menunggu') === 'menunggu')
                                            <button wire:click="$dispatch('openJadwalModal', { jadwalId: {{ $jadwal->id ?? 0 }} })" class="px-4 py-2 text-sm bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors duration-200 font-medium">
                                                <i class="fas fa-check-circle mr-1"></i>Review
                                            </button>
                                        @else
                                            <button wire:click="$dispatch('openJadwalModal', { jadwalId: {{ $jadwal->id ?? 0 }} })" class="px-4 py-2 text-sm bg-gray-600 hover:bg-gray-700 text-white rounded-lg transition-colors duration-200 font-medium">
                    ...
                    <!-- Tab: Calendar View -->
                    <div id="calendar-tab" class="jadwal-tab-content hidden bg-white rounded-xl shadow-sm p-6">
        ...
        function filterJadwalStatus(status) {
            ...
            document.querySelectorAll('.filter-btn').forEach(btn => {
                if (btn.dataset.status === status) {
                    ...
                    btn.classList.remove('bg-white', 'text-gray-700', 'border-gray-300', 'hover:border-yellow-300', 'hover:border-green-300', 'hover:border-red-300');
                } else {
