...
<body class="bg-purple-50 text-gray-800">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto p-6">
                <div class="max-w-7xl mx-auto">
                    <div id="migrationWarning" class="mb-4 p-3 bg-red-100 text-red-800 rounded {{ isset($schemaChecks) && $schemaChecks['dosen_penguji_ready'] ? 'hidden' : ''}}">
                        Skema database untuk penguji belum diperbarui. Jalankan migrasi (`php artisan migrate`).
                        @if (!isset($schemaChecks) || !$schemaChecks['dosen_penguji_ready'])
                        ...
                        <div class="bg-white rounded-lg shadow-sm p-6 border-l-4 border-purple-600">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold text-purple-900" id="ujianTotal">{{ $ujianTotal ?? 0 }}</p>
                                </div>
                        ...
                        <div class="bg-white rounded-lg shadow-sm p-6 border-l-4 border-yellow-600">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold text-yellow-900" id="ujianMenungguNilai">{{ $ujianMenungguNilai ?? 0 }}</p>
                                </div>
                        ...
                        <div class="bg-white rounded-lg shadow-sm p-6 border-l-4 border-blue-600">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold text-blue-900" id="ujianMendatang">{{ $ujianMendatang ?? 0 }}</p>
                                </div>
                        ...
                        <div class="bg-white rounded-lg shadow-sm p-6 border-l-4 border-green-600">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold text-green-900" id="rataRataNilai">{{ $rataRataNilai ?? 0 }}</p>
                                </div>
                                        ...
                                        <tr class="border-b hover:bg-gray-50">
                                            ...
                                            <td class="py-3 px-2">
                                                <span class="px-3 py-1 rounded-full text-xs font-medium
                                                    @if($ujian['status_key'] == 'selesai') bg-green-100 text-green-800
                                                    @elseif($ujian['status_key'] == 'berlangsung') bg-blue-100 text-blue-800
                                                    @else bg-yellow-100 text-yellow-800 @endif">
                                                    {{ $ujian['status_label'] ?? ucfirst(str_replace('_', ' ', $ujian['status'])) }}
                                                </span>
                                            </td>
    ...
    <script>
        ...
        function refreshData() {
            fetch('{{ route("dosen_penguji.dashboard.data") }}', {
                method: 'GET',
                headers: {
        ...
        document.addEventListener('DOMContentLoaded', () => {
            const initialData = {
                ujianTotal: {{ $ujianTotal ?? 0 }},
                ujianMenungguNilai: {{ $ujianMenungguNilai ?? 0 }},
                ujianMendatang: {{ $ujianMendatang ?? 0 }},
                rataRataNilai: {{ $rataRataNilai ?? 0 }},
                ujianList: @json($ujianList ?? []),
