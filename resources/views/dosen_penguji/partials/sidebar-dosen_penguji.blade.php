...
<aside class="w-64 bg-white shadow-md flex flex-col" id="sidebar">
    ...
    <nav class="flex-1 overflow-y-auto py-4">
        <ul class="space-y-1 px-3">
            <li>
                <a href="{{ route('dosen_penguji.dashboard') }}"
                   class="flex items-center space-x-3 px-4 py-3 rounded-lg font-medium transition 
                   {{ request()->routeIs('dosen_penguji.dashboard') ? 'bg-purple-100 text-purple-700 font-semibold' : 'text-gray-700 hover:bg-purple-50' }}">
                    <i class="fas fa-home w-5"></i><span>Dashboard</span>
                </a>
            </li>
            <li>
                <a href="{{ route('dosen_penguji.penilaian') }}"
                   class="flex items-center space-x-3 px-4 py-3 rounded-lg transition
                   {{ request()->routeIs('dosen_penguji.penilaian') ? 'bg-purple-100 text-purple-700 font-semibold' : 'text-gray-700 hover:bg-purple-50' }}">
