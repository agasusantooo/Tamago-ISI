...
<body class="bg-green-50 text-gray-800">
    <div class="flex h-screen overflow-hidden">
        ...
        @include('koordinator_story_conference.partials.sidebar-koordinator')
                        ...
                        <div class="col-span-8">
                            <div class="bg-white rounded-xl shadow p-4">
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <span class="text-xs text-gray-500">{{ collect($pending['story_conference'] ?? [])->count() }} Pending</span>
                                </div>

                                <div class="space-y-3">
                                    @foreach($pending['story_conference'] as $s)
                                    <div class="flex items-center justify-between bg-gray-50 p-3 rounded">
                                            <div>
                                                <div class="text-sm font-medium">Pendaftaran Story Conference</div>
                                                <div class="text-xs text-gray-600">Mahasiswa: {{ optional(optional($s->resolved_mahasiswa)->user)->name ?? optional($s->resolved_mahasiswa)->nama ?? 'N/A' }} ({{ optional($s->resolved_mahasiswa)->nim ?? 'N/A' }})</div>
                                            </div>
                                        <div class="flex items-center space-x-2">
                                            <a href="{{ route('koordinator_story_conference.monitoring') }}" class="px-3 py-1 bg-green-600 text-white rounded">✓</a>
                                            <a href="{{ route('koordinator_story_conference.monitoring') }}" class="px-3 py-1 bg-red-600 text-white rounded">✕</a>
                                        </div>
                                    </div>
                                    @endforeach
                                </div>
