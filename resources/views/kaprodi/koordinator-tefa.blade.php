...
@section('title', 'Setup Koordinator Koordinator Prodi')
@section('page-title', 'Setup Koordinator')
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        <div class="grid grid-cols-2 gap-6">
            <div>
                            ...
                            <select id="user_id_tefa" name="user_id" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-teal-500 focus:border-teal-500 sm:text-sm">
                                ...
                                @foreach($dosens as $dosen)
                                    <option value="{{ $dosen->user->id }}" 
                                            {{ optional($currentCoordinatorTefa)->id == $dosen->user->id ? 'selected' : '' }}>
                                        {{ $dosen->user->name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                @if($currentCoordinatorTefa)
                <div class="mt-8 pt-6 border-t">
                    <h4 class="text-md font-semibold text-gray-700">Koordinator TEFA Saat Ini</h4>
                    <p class="text-lg text-teal-700 font-bold mt-2">{{ $currentCoordinatorTefa->name }}</p>
                </div>
                @endif
            </div>
            ...
            <div>
                ...
                <form action="{{ route('kaprodi.koordinator-story-conference.update') }}" method="POST">
                    @csrf
                    <div class="space-y-4">
                        <div>
                            ...
                            <select id="user_id_story" name="user_id" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                ...
                                @foreach($dosens as $dosen)
                                    <option value="{{ $dosen->user->id }}" 
                                            {{ optional($currentCoordinatorStory)->id == $dosen->user->id ? 'selected' : '' }}>
                                        {{ $dosen->user->name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                @if($currentCoordinatorStory)
                <div class="mt-8 pt-6 border-t">
                    <h4 class="text-md font-semibold text-gray-700">Koordinator Story Conference Saat Ini</h4>
                    <p class="text-lg text-indigo-700 font-bold mt-2">{{ $currentCoordinatorStory->name }}</p>
                </div>
                @endif
            </div>
        </div>
